function F = Box_12(X1,X2,X3,Name1)
...
group50 = [zeros(length(X2(:,1)),1)'+4,zeros(length(X2(:,2)),1)'+5,zeros(length(X2(:,3)),1)'+6,...
    zeros(length(X2(:,4)),1)'+7];
group90 = [zeros(length(X3(:,1)),1)'+8,zeros(length(X3(:,2)),1)'+9,zeros(length(X3(:,3)),1)'+10,...
...
positions = [1.25 1.5 1.75 2 2.5 2.75 3 3.25 3.75 4 4.25 4.5];
...
for j=[1,5,9] % it ll take revesre order
...
for j=[3,7,11] % it ll take revesre order
    hP = patch(get(h(j),'XData'),get(h(j),'YData'),'g','FaceAlpha',.5,'EdgeColor','none');
...
for j=[4,8,12] % it ll take revesre order
...
set(gca,'xTick',[1.625,2.8750, 4.1250])
